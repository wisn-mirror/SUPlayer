package com.suplayer.tv.local.model;
...
/**
 * Created by Wisn on 2019-08-07 14:43.
 */
public class MediaInfoModel {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (containsVideo) {
                    ...
                    callback.onSuccessFolder(folders);
                } else {
                    Collections.reverse(allImage);
                    callback.onSuccessFolder(splitFolder(allImage));
                }
            }
        }).start();
    }
    ...
    public static void getAllVideoInfos(final Context context, final DataCallback callback) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ArrayList<MediaInfo> allVideo = getAllVideo(context);
                callback.onSuccessMediaInfo(allVideo);
                Collections.reverse(allVideo);
                callback.onSuccessFolder(splitFolder(allVideo));
            }
        }).start();
    }

    private static ArrayList<MediaInfo> getAllImage(Context context) {
        //扫描图片
    ...
    private static ArrayList<MediaInfo> getAllVideo(Context context) {
        //扫描视频
        ...
        if (allVideo != null && allVideo.moveToFirst()) {
            while (!allVideo.isAfterLast() && allVideo.moveToNext()) {
                ...
                if (!"downloading".equals(getExtensionName(path))) { //过滤未下载完成的文件
                    images.add(new MediaInfo(path, time, name, mimeType, true, getTimeDurationStr(duration), size));
                }
            }
            allVideo.close();
        }
    ...
    public interface DataCallback {
        void onSuccessFolder(ArrayList<Folder> folders);
        void onSuccessMediaInfo(ArrayList<MediaInfo> mediainfo);
    }
