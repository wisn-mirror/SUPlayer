...
import com.wisn.suvideo.control.MediaPlayerControl;
...
import com.wisn.suvideo.player.mediaplayer.AndroidMediaPlayerFactory;
import com.wisn.suvideo.view.IRenderView;
import com.wisn.suvideo.view.SurfaceRenderView;
import com.wisn.suvideo.view.TextureRenderView;
...
/**
 * Created by Wisn on 2019-07-23 16:11.
 */
public class SuVideoView extends FrameLayout implements MediaPlayerControl, PlayerEventListener {
    ...
    public SuVideoView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SuVideoView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//            PlayerUtils.showActionBar(activity);
    ...
    public void startTinyScreen(boolean isCustomer, ViewGroup viewGroup) {
        ...
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            @Override
            public void run() {
                SuVideoView.this.removeView(mPlayerContainer);
                ...
                if (isCustomer) {
                    ...
                    viewGroup.addView(mPlayerContainer, params);
...
//                    layoutParams.topMargin = PlayerUtils.dp2px(mContext, 50);
                    ...
                    params.topMargin = PlayerUtils.dp2px(mContext, 50);
                    viewGroup.addView(mPlayerContainer, params);
                }
                mIsTinyScreen = true;
    ...
    public void startTinyScreen() {
        ...
        if (activity == null) return;
        ViewGroup contentView = activity.findViewById(android.R.id.content);
        startTinyScreen(false, contentView);
     ...
     * 退出小屏
     */
    public void stopTinyScreen(ViewGroup viewGroup) {
        if (!mIsTinyScreen) return;
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            @Override
            public void run() {
                ...
                LayoutParams params = new LayoutParams(
                        ViewGroup.LayoutParams.MATCH_PARENT,
                        ViewGroup.LayoutParams.MATCH_PARENT);
                SuVideoView.this.addView(mPlayerContainer, params);
